...
with open('kodaikanal.html', 'r', encoding='utf-8') as f:
    template = f.read()
...
for p in places:
    ...
    # Meta Titles
    output = re.sub(r'<title>.*?</title>', f"<title>{p['title']} - Famous Destinations | AUTOBOTZ</title>", output)
    
    # Hero Bg Image
    output = output.replace('src="images/places/kodaikanal1.png"', f'src="images/places/{p["image"]}"')
    
    # Tag, Title, Tagline
    output = re.sub(r'<span class="hero-tag">.*?</span>', f'<span class="hero-tag"><i class="fa fa-map-marker-alt" style="margin-right: 8px;"></i> {p["tag"]}</span>', output)
    output = re.sub(r'<h1 class="hero-title" id="dy-title">.*?</h1>', f'<h1 class="hero-title" id="dy-title">{p["title"]}</h1>', output)
    output = re.sub(r'<p class="hero-tagline" id="dy-tagline">.*?</p>', f'<p class="hero-tagline" id="dy-tagline">{p["tagline"]}</p>', output)
    
    # About Text
    output = re.sub(r'<h2 id="dy-about-title">.*?</h2>', f'<h2 id="dy-about-title">{p["aboutTitle"]}</h2>', output)
    output = re.sub(r'<div id="dy-about-desc">.*?</div>', f'<div id="dy-about-desc">{p["aboutDesc"]}</div>', output, flags=re.DOTALL)
    
    # Stats
    output = re.sub(r'<strong id="dy-dist">.*?</strong>', f'<strong id="dy-dist">{p["dist"]}</strong>', output)
    output = re.sub(r'<strong id="dy-time">.*?</strong>', f'<strong id="dy-time">{p["time"]}</strong>', output)
    
    # About Image
    output = output.replace('src="images/places/kodaikanal2.png"', f'src="https://loremflickr.com/800/600/{p["query"]}?random=9"')
    output = output.replace('alt="Kodaikanal Pine Forest"', f'alt="{p["title"]} View"')

    # Highlights
    highlightHtml = f"""
                <div class="highlight-item">
                    <div class="highlight-icon"><i class="fa-solid {p['high1Icon']}"></i></div>
                    <h3>{p['high1Title']}</h3>
                    <p>{p.get('high1Desc', '')}</p>
                </div>
                
                <div class="highlight-item">
                    <div class="highlight-icon"><i class="fa-solid {p['high2Icon']}"></i></div>
                    <h3>{p['high2Title']}</h3>
                    <p>{p.get('high2Desc', '')}</p>
                </div>
                
                <div class="highlight-item">
                    <div class="highlight-icon"><i class="fa-solid {p['high3Icon']}"></i></div>
                    <h3>{p['high3Title']}</h3>
                    <p>{p.get('high3Desc', '')}</p>
                </div>"""
                
    output = re.sub(r'<div class="highlights-grid">.*?</section>', f'<div class="highlights-grid">{highlightHtml}</div></div></section>', output, flags=re.DOTALL)

    # Gallery
    galleryHtml = f"""
        <div class="gallery-grid">
            <div class="gallery-item">
                <img src="images/places/{p['image']}" alt="{p['title']}">
                <div class="gallery-caption">
                    <h3>{p['title']} Panorama</h3>
                    <p>Beautiful scenic view.</p>
                </div>
            </div>
            <div class="gallery-item">
                <img src="https://loremflickr.com/600/400/{p['query']}?random=1" alt="{p['title']} Snapshot">
                <div class="gallery-caption">
            ...
            <div class="gallery-item">
                <img src="https://loremflickr.com/600/400/{p['query']}?random=2" alt="{p['title']} Detail">
                <div class="gallery-caption">
    ...
    output = re.sub(r'<div class="gallery-grid">.*?</div>\s*<!-- WHY CHOOSE US -->', f'{galleryHtml}\n\n        <!-- WHY CHOOSE US -->', output, flags=re.DOTALL)

    # CTA
    output = re.sub(r'<h2 id="dy-cta">.*?</h2>', f'<h2 id="dy-cta">Ready to explore {p["title"]}?</h2>', output)

    with open(p['file'], 'w', encoding='utf-8') as dest:
        dest.write(output)
